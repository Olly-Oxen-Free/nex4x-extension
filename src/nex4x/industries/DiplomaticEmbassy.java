...
public class DiplomaticEmbassy extends BaseIndustry {
    ...
    @Override
    public float getPatherInterest() { return 0f; }
    ...
    protected void addPostDescriptionSection(TooltipMakerAPI tip, Industry.IndustryTooltipMode mode) {
        ...
        try {
            ...
            influence = inf.getJSONObject("buildingIncome").optInt(Nex4xConstants.BUILDING_DIPLOMATIC_EMBASSY, 4);
            JSONObject sec = Global.getSettings().loadJSON(Nex4xConstants.PATH_MARKET_SECURITY);
            assist = (float) sec.optDouble("embassyAssistBonus", 3.0);
        } catch (Exception ignore) {}

        tip.addPara("Influence income: +%s per cycle", pad,
                Misc.getHighlightColor(), String.valueOf(influence));
        tip.addPara("Counter-espionage assist: +%s", 3f,
